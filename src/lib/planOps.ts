...
export function applyOp(plan: BuildingPlanModel, op: PlanOp): BuildingPlanModel {
  switch (op.type) {
    ...
    case 'upsert_openings_batch': {
      let nextOpenings = plan.openings.slice();
      for (const raw of op.openings) {
        ...
        nextOpenings = idx >= 0 ? replaceAt(nextOpenings, idx, opening) : [...nextOpenings, opening];
      }
      return bumpRev({ ...plan, openings: nextOpenings });
    }
    case 'delete_opening': {
    ...
    case 'upsert_stair': {
      ...
      const idx = stairs.findIndex((s) => s.id === op.stair.id);
      const next = idx >= 0 ? replaceAt(stairs, idx, op.stair) : [...stairs, op.stair];
      return bumpRev({ ...plan, stairs: next });
    }
    case 'delete_stair': {
      const stairs = plan.stairs ?? [];
      return bumpRev({ ...plan, stairs: stairs.filter((s) => s.id !== op.stairId) });
    }
    case 'upsert_fixture': {
