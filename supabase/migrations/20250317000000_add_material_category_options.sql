-- Store which categories are optional per sheet (no change to material_items required).
-- Use this so the app works even when material_items.is_optional is missing or schema cache is stale.
