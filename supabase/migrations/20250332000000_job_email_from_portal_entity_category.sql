-- Ensure customer portal messages use entity_category = 'customer' (skip entirely if job_emails missing).
DO $migrate$
BEGIN
  IF to_regclass('public.job_emails') IS NULL THEN
    RETURN;
  END IF;
...
$sql$;
  ...
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.create_job_email_from_customer_portal(text, uuid, text, text) TO anon';
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.create_job_email_from_customer_portal(text, uuid, text, text) TO authenticated';
END $fn$;
