-- When the customer signs in the portal, lock working material_workbooks for that quote (status only).
...
COMMENT ON FUNCTION public.customer_sign_proposal(text, uuid, text, text) IS
  'Customer signs proposal in portal; sets signature, signed_version, and locks working material_workbooks (no row content changes).';
