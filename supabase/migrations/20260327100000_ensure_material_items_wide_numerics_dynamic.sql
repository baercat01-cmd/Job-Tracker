-- Replace ensure_material_items_wide_numerics: widen EVERY numeric column on material_items (not a fixed list).
...
COMMENT ON FUNCTION public.ensure_material_items_wide_numerics() IS
  'Widens all numeric columns on material_items to unconstrained numeric (fixes Excel upload overflow).';
